...
**Definition.** Every phone on every address of the record equals a "concentration number": a phone appearing on ≥1% of records **or** ≥50 records in the same provider file (whichever is larger). Sub-flag `NPPES_DIVERGENT_PHONE` (E3) when NPPES lists a different practice phone for that NPI. **Not implemented as a scored sub-flag in rules v0.1.** The divergence is computed and recorded inside the evidence row's `observed` field, but it does not add weight, so no published score depends on it.
